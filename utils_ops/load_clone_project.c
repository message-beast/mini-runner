...
[[nodiscard]]__attribute__((hot)) int loadCloneProject(service*** __restrict__ services, char* __restrict__  githubRepo, char* __restrict__ nickName, pid_t pid) {
    ...
    newService->pid = pid;
    if (__builtin_expect(capacityOfCloneServices <= numberOfCloneProjects, 0)) {
        service** tmp = realloc(*services, (capacityOfCloneServices + __INITIAL_SCALE_SIZE_OF_SERVICES__) * (sizeof(service*)));
        if (__builtin_expect(tmp == NULL, 0)) {
            perror("can not resize memory for additional of services");
            exit_program(-1)
        }
        (*services) = tmp;
        capacityOfCloneServices += __INITIAL_SCALE_SIZE_OF_SERVICES__;
    }
