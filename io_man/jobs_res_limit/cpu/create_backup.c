...
static inline __attribute__((always_inline, hot)) int createJobLmBackup(char* cpuLimitBuff) {
    int fd = open(_JOB_CPU_LIMIT_COPY_FILE, O_CREAT | O_RDWR, 0644);
...
int createJobRsLimitBackup() {
    if (__builtin_expect(doBackup(_JOB_CPU_LIMIT_FILE, _JOB_CPU_LIMIT_FILE) != 0, 0)) {
        return -1;
    }
    return 0;
}